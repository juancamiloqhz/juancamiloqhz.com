...
- Use Zod for schema validation (frontmatter, configs)
...
Environment variables are defined in `.env.local` (local dev) and configured in deployment platform (Vercel). No runtime validation layer — variables are accessed directly via `process.env`.

Required variable: `NEXT_PUBLIC_APP_URL` (used by next-sitemap and RSS generation).
...
| `clsx` + `tailwind-merge`  | Class name utilities (used by `cn()`)        |
| `framer-motion`            | Animations                                   |
| `next-themes`              | Dark/light/system theme                      |
| `next-view-transitions`    | Page transition animations                   |
| `next-sitemap`             | Sitemap generation (postbuild)               |
| `rss`                      | RSS feed generation (postbuild)              |

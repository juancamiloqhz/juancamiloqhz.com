import { PostLayout } from '../../../components';

export const meta = {
  title: 'Featured Post 1',
  description:
    'lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.',
  date: 'June 04, 2012',
  readTime: 10,
  author: 'Juan Camilo QHz',
  featured: true,
  image:
    '/photo-1531297484001-80022131f5a1?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1200&q=80',
};

export default ({ children }) => (
  <PostLayout meta={meta}>{children}</PostLayout>
);

## My Headline

Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque maximus pellentesque dolor non egestas. In sed tristique elit. Cras vehicula, nisl vel ultricies gravida, augue nibh laoreet arcu, et tincidunt augue dui non elit. Vestibulum semper posuere magna, quis molestie mauris faucibus ut.

Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque maximus pellentesque dolor
